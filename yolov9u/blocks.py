...
def autopad(k, p=None, d: int = 1):  # kernel, padding, dilation
...
class DDetect(nn.Module):
    ...
    def __init__(self, class_count=80, ch=(), inplace=True):  # detection layer
        super().__init__()
        self.class_count = class_count  # number of classes
        self.detection_layer_count = len(ch)  # number of detection layers
        self.reg_max = 16
        self.no = class_count + self.reg_max * 4  # number of outputs per anchor
        self.inplace = inplace  # use inplace ops (e.g. slice assignment)
        self.stride = torch.zeros(
            self.detection_layer_count
        )  # strides computed during build

        c2, c3 = make_divisible(max((ch[0] // 4, self.reg_max * 4, 16)), 4), max(
            (ch[0], min((self.class_count * 2, 128)))
        )  # channels
        ...
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.class_count, 1)
            )
            for x in ch
        )
    ...
    def forward(self, x):
        ...
        for i in range(self.detection_layer_count):
        ...
        box, cls = torch.cat([xi.view(shape[0], self.no, -1) for xi in x], 2).split(
            (self.reg_max * 4, self.class_count), 1
        )
        dbox = (
    ...
    #         b[-1].bias.data[: m.class_count] = math.log(
    #             5 / m.class_count / (640 / s) ** 2
